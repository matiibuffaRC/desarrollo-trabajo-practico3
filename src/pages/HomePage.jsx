            ...
            <section className="mx-auto max-w-6xl">
                ...
                {favorites.length > 0 && (
                    <div className="mt-8 mb-6">
                        <h2 className='text-2xl font-bold m-2 text-[#1DB954] mb-4'>❤️ Mis Artistas Favoritos</h2>
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {favorites.map((fav) => (
                                <div 
                                    key={fav.id} 
                                    onClick={() => navigate(`/artist/${fav.id}?name=${encodeURIComponent(fav.name)}`)} 
                                    className="cursor-pointer relative group transform hover:-translate-y-1 transition-transform"
                                >
                                    <ArtistCard
                                        id={fav.id} 
                                        name={fav.name}
                                        genre={fav.genres?.slice(0, 2).join(' • ') || 'Favorito'}
